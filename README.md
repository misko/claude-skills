...
Audit + fix discipline for any structured artifact (papers, projects, experiments, datasets, ADRs, runbooks, anything with frontmatter + structured prose). One self-contained `.md` file — give it to Claude with a workspace path and you get the full audit + fix loop.

You author one `<name>_contract.md` per artifact kind; the skill runs three orthogonal stages:
